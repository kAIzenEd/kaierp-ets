...
class ResConfigSettings(models.TransientModel):
    ...
    whatsapp_event_rule_count = fields.Integer(
        compute='_compute_whatsapp_event_rule_count',
    ...
    @api.depends()
    def _compute_whatsapp_event_rule_count(self):
        count = self.env['school.whatsapp.event.rule'].search_count([])
        for rec in self:
            rec.whatsapp_event_rule_count = count
    ...
    def action_whatsapp_open_event_rules(self):
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
            'name': _('WhatsApp Notification Rules'),
            'res_model': 'school.whatsapp.event.rule',
            'view_mode': 'list,form',
            'target': 'current',
        }
